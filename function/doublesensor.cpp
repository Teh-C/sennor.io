...
DoubleSensor::DoubleSensor(QWidget *parent) :
    ...
    xCheckBox(new QCheckBox(tr("x轴"))),
    yCheckBox(new QCheckBox(tr("y轴")))
{
    ui->setupUi(this);
    ...
    checkBoxLayout->addWidget(xCheckBox);
    checkBoxLayout->addWidget(yCheckBox);
    btnLayout->addLayout(checkBoxLayout);
    ...
    connect(xCheckBox, SIGNAL(stateChanged(int)), this, SLOT(dealxCheckBoxStatus(int)));
    connect(yCheckBox, SIGNAL(stateChanged(int)), this, SLOT(dealyCheckBoxStatus(int)));
...
void DoubleSensor::deleteLastAngleValue()
{
 ...
 *****************************************************************************/
void DoubleSensor::GetAngleValue()
{
...
void DoubleSensor::countDiffValue()
{
...
void DoubleSensor::getItemData(QSqlQuery &query,QString str, QStandardItemModel *model, int row, int column)
{
    if(model->data(model->indexFromItem(model->item(row, column))).isValid())
        query.bindValue(str, model->item(row,column)->text().toDouble());
}

void DoubleSensor::saveFile()
{
...
void DoubleSensor::setMainTab(MainTab *mainTab)
{

    this->mainTab = mainTab;
}

void DoubleSensor::dealxCheckBoxStatus(int state)
{
    if(Qt::CheckState(state) == Qt::Checked)
    {
        yCheckBox->setCheckState(Qt::Unchecked);
    }
    else if(Qt::CheckState(state) == Qt::Unchecked)
    {
        yCheckBox->setCheckState(Qt::Checked);
    }
}

void DoubleSensor::dealyCheckBoxStatus(int state)
{
    if(Qt::CheckState(state) == Qt::Checked)
    {
        xCheckBox->setCheckState(Qt::Unchecked);
    }
    else if(Qt::CheckState(state) == Qt::Unchecked)
    {
        xCheckBox->setCheckState(Qt::Checked);
